...
if input_option == "Fill out a Form":
    ...
    if st.button('Predict'):
        ...
        if response.status_code == 200:
            ...
            if prediction == 1:
                st.markdown(f'<div class="prediction-box prediction-default">Prediction: Default<br>Confidence: {round(((1 - probability) * 100),2)}%</div>', unsafe_allow_html=True)
            else:
                st.markdown(f'<div class="prediction-box prediction-fully-paid">Prediction: Fully Paid<br>Confidence: {round(((1 - probability) * 100),2)}%</div>', unsafe_allow_html=True)
        else:
